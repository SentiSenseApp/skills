...
- Any HTTP client, or no install at all via the official CLI (`npx -y sentisense@0.45.0`). Plain `curl` works, or Python 3.8+ using only the standard library.
...
```bash
npx -y sentisense@0.45.0 insiders NVDA --days 90
npx -y sentisense@0.45.0 insiders NVDA --days 180 --json
npx -y sentisense@0.45.0 insiders NVDA --full     # all rows in plain output, not the top 15
```

Plain output in a terminal, exact API JSON with `--json` (envelope included). Auth: `SENTISENSE_API_KEY` in the environment, or store it once with `npx -y sentisense@0.45.0 auth "$SENTISENSE_API_KEY"` (saved to `~/.config/sentisense/`, file mode 600, local to your machine, removable with `auth --remove`). The version is pinned deliberately: a pinned version runs reviewed, immutable code.
...
```bash
npx -y sentisense@0.45.0 insiders NVDA --days 90 --json
```

REST equivalent: `GET /api/v1/insider/trades/NVDA?lookbackDays=90`. Filter to the directional rows first (code `P` and code `S`, minus code `F`), then report: who bought and sold, their roles, net dollars, and how much of the selling was 10b5-1 planned. **Compute the dollar figures yourself from the `--json` rows**: the CLI's `bought:`/`sold:` summary line is a raw type tally that currently still includes code-F withholding dollars, so quote your filtered sum, not the headline. An all-award window is zero insider conviction either way, not a wave of it; say "no open-market insider activity" rather than presenting awards as trades.
