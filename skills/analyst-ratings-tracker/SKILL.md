...
- **Consensus counts are a monthly panel, not a tally of the actions feed.** The `strongBuy` to `strongSell` counts are published in calendar-month buckets and can shift on the first sweep of a new month with no matching row on `/actions` or `/activity`; a change published mid-month shows on `/activity` first. Never explain a count move by hunting for an action, and never say "no analyst moved" because the counts are flat. The actions feed is the event record (each row has the analyst's own `actionDate`); the counts are a periodic snapshot.
