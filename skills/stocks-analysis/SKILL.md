...
```bash
npx -y sentisense@0.51.0 health              # reachability, key validity, latency, one call
...
1. `npx -y sentisense@0.51.0 quote {T} --json` for price + day change (`currentPrice`, `changePercent`). REST: `GET /api/v1/stocks/price?ticker={T}`
...
3. `npx -y sentisense@0.51.0 insiders {T} --days 90 --json` for insider activity (`.data[]`). REST: `GET /api/v1/insider/trades/{T}?lookbackDays=90`
4. `npx -y sentisense@0.51.0 analysts {T} --json` for the target band (`.consensus.data`). REST: `GET /api/v1/analyst/{T}/consensus`
5. `npx -y sentisense@0.51.0 insights {T} --json` for AI insights (`.data[]`; take the first item for the headline, check its `generatedAt` and flag age). REST: `GET /api/v1/insights/stock/{T}`
...
2. `npx -y sentisense@0.51.0 congress --days 7 --limit 50 --json` (`.data[]`, filter to PURCHASE). REST: `GET /api/v1/politicians/activity?lookbackDays=7`
...
6. `npx -y sentisense@0.51.0 analysts {T} --days 30 --json` for rating changes (`.actions.data[]`; the same call also carries `.consensus.data`, so it covers the analyst band for free). REST: `GET /api/v1/analyst/{T}/actions?lookbackDays=30`
...
```bash
npx -y sentisense@0.51.0 quote {T} --json              # E1  price, day change
npx -y sentisense@0.51.0 sentiment {T} --json          # E11 .sentiment.data.sentisenseScore
npx -y sentisense@0.51.0 insiders {T} --days 90 --json # E12 .data[]
npx -y sentisense@0.51.0 congress {T} --days 90 --json # E13 .data[]
npx -y sentisense@0.51.0 flows {T} --json              # E14 .data.holders[], quarter resolved
npx -y sentisense@0.51.0 analysts {T} --json           # E15 .consensus.data
npx -y sentisense@0.51.0 mood --json                   # E17 .market
npx -y sentisense@0.51.0 options {T} --json            # E19 .data, optional
```
...
- **Sample size matters on sentiment rows.** A reading built on a handful of mentions is noise, not signal. Get the day's directional mention count with the thin-sample guard in Quick Read 3 and apply it to E10 and E11; `npx -y sentisense@0.51.0 sentiment {T} --json` also carries the day's total at `.sentiment.data.mentions` as a coarse cross-check. Note thin samples in the Value cell ("+0.41 on 5 mentions, thin") and expect them to be attacked in R2.
...
Commands take the prefix `npx -y sentisense@0.51.0`, then:
