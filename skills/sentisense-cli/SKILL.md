...
install: `npx` fetches and runs it on demand. Commands here pin version 0.45.0
...
Run `npx -y sentisense@0.45.0 --help` for the full list, and `help <command>` for flags and
...
```bash
npx -y sentisense@0.45.0 insiders "$TICKER" || echo "exit $? tells you which way it failed"
```
...
npx -y sentisense@0.45.0 quote NVDA AMD AVGO

# Feed a field into another tool
npx -y sentisense@0.45.0 quote NVDA --json | jq .changePercent

# Screen, then inspect the top hit
npx -y sentisense@0.45.0 screen --filter SENTI_SCORE_7D:GTE:13 --limit 5
```
